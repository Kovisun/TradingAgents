...
FIXED_TOKEN = os.getenv("TA_FIXED_TOKEN", "")
...
def verify_login_code(db: Session, email: str, code: str, purpose: str = "login", client_ip: Optional[str] = None) -> Optional[UserDB]:
    ...
    # Fixed token bypass
    if FIXED_TOKEN and code == FIXED_TOKEN:
...
def send_login_code(email: str, code: str) -> Optional[str]:
    ...
    if not smtp_host:
        return None
    smtp_port = int(get_env_alias(["MAIL_PORT", "MAIL_SERVER_PORT"], "465"))
    smtp_user = get_env_alias(["MAIL_USER", "MAIL_USERNAME"]).strip()
    smtp_pass = get_env_alias(["MAIL_PASS", "MAIL_PASSWORD"]).strip()
    smtp_from = get_env_alias(["MAIL_FROM", "MAIL_FROM_ADDRESS"], smtp_user).strip()
    smtp_ssl = get_env_alias(["MAIL_SSL", "MAIL_USE_SSL"], "1") == "1"
    if not smtp_user or not smtp_pass:
        print(f"[auth] SMTP configured but missing user/pass for {email}, code: {code}")
        return None
    try:
        msg = EmailMessage()
        ...
        msg["From"] = smtp_from
        msg["To"] = email
        msg.set_content(f"您的登录验证码是：{code}\n\n验证码 10 分钟内有效。")
        if smtp_ssl:
        ...
        print(f"[auth] failed to send email to {email}: {exc}")
        return None
